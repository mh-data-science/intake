...
def merge_pars(params, user_inputs, spec_pars, client=False, getenv=True, getshell=True):
    ...
    # Hi, basically, if you have default account_name: {{param}}acc then intake decides to only param interpolate once.
    # So it does it in default args but then if  you pass it in storage ops it fails ...
    # So instead don't pass it in storage ops and just take it from default args then.
    if ("account_name" not in params.get("storage_options", {}) 
        and "account_name" in out.get("storage_options", {}) 
        and params.get("urlpath", "").startswith("abfs://")):
        params["storage_options"]["account_name"] = out["storage_options"]["account_name"]
